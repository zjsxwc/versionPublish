...
var FileUploadComponent = Vue.extend({
  template: '<div class="{{ class }}"><label for="{{ name }}"><input type="file" name="{{ name }}" id="{{ id || name }}" accept="{{ accept }}" v-on:click="fileInputClick" v-on:change="fileInputChange" multiple="{{ multiple }}" v-show="!(hideComponent == \'true\')"><slot></slot></label><button type="button" v-on:click="fileUpload" v-show="!(hideComponent == \'true\')">Upload</button></div>',
  props: {
    class: String,
    name: {
      type: String,
  ...
  methods: {
    ...
    fileInputChange: function() {
      ...
      if (this.uploadImgOnFileChange === "true") {
        this.fileUpload();
      }
    },
        ...
        xhr.onreadystatechange = function() {
          ...
          if (xhr.status < 400) {
            ...
            if (this.imgUploadedCustomEvent){
              this.$dispatch(this.imgUploadedCustomEvent, file, res);
            }
