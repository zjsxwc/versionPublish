function BigUpload (settings) {
	...
	if (settings.scriptPath) {
		this.settings.scriptPath = settings.scriptPath;
	}
	if (settings.inputField) {
		this.settings.inputField = settings.inputField;
	}
	if (settings.submitButton) {
		this.settings.submitButton = settings.submitButton;
	}
	if (settings.responseField) {
		this.settings.responseField = settings.responseField;
	}
	if (settings.progressBarField) {
		this.settings.progressBarField = settings.progressBarField;
	}
	if (settings.timeRemainingField) {
		this.settings.timeRemainingField = settings.timeRemainingField;
	}
	...
	this.sendFileData = function() {
		var fileName = this.$(this.settings.inputField).getAttribute("file-name");
		if (!fileName) {
			fileName = this.uploadData.file.name;
		}
		var data = 'key=' + this.uploadData.key + '&name=' + fileName;
		xhr = new XMLHttpRequest();
